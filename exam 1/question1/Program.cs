...
class Program
{
    ...
    start:
        ...
        for (nCntr = 0; nCntr < nQuestions; ++nCntr)
        {
            ...
            nOp = rand.Next(0, 4); // expand to 4 instead of 3
            ...
            else // add another case for division
            {
                nAnswer = Math.Round((val1 / val2), 2); // round answer
                sQuestions = $"Question #{nCntr + 1}: {val1} / {val2} => (Round to the 2nd Deimal):";
            }
            ...
            do
            {
                ...
                try
                {
                    nResponse = Convert.ToDouble(sResponse); // convert to DOUBLE
                    bValid = true;
                }
                catch
                {
